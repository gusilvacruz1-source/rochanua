---
name: rocha-nua
description: Identidade visual e estrutura do site da banda Rocha Nua (@rocha.nua). Usar em qualquer edição de layout, cor, tipografia, componente ou texto neste repositório.
---

# Site da Rocha Nua

Repositório próprio, só da banda. O site fica na **raiz**. HTML, CSS e
JS puros, sem build, sem framework, sem biblioteca de componentes.
...
As fotos que a banda enviou vivem em `_fotos-originais/`
...
como visual próprio faz parecer que são da banda. Não estão neste
repositório e não devem entrar. Também não recortar esses logos para
virar marca da Rocha Nua.
